...
sites = np.unique([os.path.basename(f).split(".")[0].split("_")[0] for f in all_files])
for site in sites:
    print(site)
    for sci_id, sci_config in enumerate(sci_configs):

        old_fname = glob.glob("%s/%s_*_R%d_S%d_out.nc" % (ofdir, site, 0, sci_id))[0]
        new_fname = glob.glob("%s/%s_*_R%d_S%d_out.nc" % (ofdir, site, 1, sci_id))[0]
